...
class Cache:
    ...
    def get(
        self, link: Link, package_name: Optional[str], supported_tags: List[Tag],
    ) -> Link:
    ...
    def get(
        ...
        for wheel_name, wheel_dir in self._get_candidates(link, canonical_package_name):
            ...
            if not wheel.supported(supported_tags):
                ...
                continue
            candidates.append(
                (wheel.support_index_min(supported_tags), wheel_name, wheel_dir,)
            )
...
class EphemWheelCache(SimpleWheelCache):
    ...
    def __init__(self, format_control: FormatControl) -> None:
        self._temp_dir = TempDirectory(
            kind=tempdir_kinds.EPHEM_WHEEL_CACHE, globally_managed=True,
        )
...
class CacheEntry:
    def __init__(
        self, link: Link, persistent: bool,
    ):
        self.link = link
